...
- (NSMutableAttributedString *)switchContentWithFonts:(NSArray *)fontArray withRanges:(NSArray *)ranges withColors:(NSArray *)colors {

    NSMutableAttributedString *attStr = [[NSMutableAttributedString alloc] initWithString:self];
    for (int i = 0; i<fontArray.count; i++) {
        UIFont *aFont = fontArray[i];
        UIColor *aColor = colors[i];
        NSString *rangeStr = ranges[i];
        NSRange strRange = NSRangeFromString(rangeStr);
        NSDictionary *attribute = [NSDictionary dictionaryWithObjectsAndKeys:aFont,NSFontAttributeName,aColor,NSForegroundColorAttributeName, nil];
        [attStr setAttributes:attribute range:strRange];
    }
    return attStr;
}
@end
